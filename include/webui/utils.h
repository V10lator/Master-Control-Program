...
static inline void httpFastReply(struct mg_connection *c, int err)
{
	...
	mg_http_reply(c, err, "Content-Type: text/html; charset=utf-8" HTTP_NEWLINE CACHE_CONTROL_NOCACHE, "<html><head><title>%i %s</title><link href=\"/css/technics.css\" rel=\"stylesheet\"></head><body><center><h1>%i %s</h1><hr>" HTTP_STRING  "</center></body></html><!-- We need some padding cause some browsers are stupid ant want to render their shit instead of this beautiful page we worked so much on... -->" HTTP_NEWLINE,
		err, str,
		err, str
	);
}
